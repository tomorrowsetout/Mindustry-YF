...
4. [模块编写指南](#4-模块编写指南)
5. [插件编写指南](#5-插件编写指南)
6. [module.hjson 完整字段说明](#6-modulehjson-完整字段说明)
7. [JavaScript API 完整参考](#7-javascript-api-完整参考)
   ...
   - [全局对象参考](#全局对象参考)
8. [外部服务连接配置指南](#外部服务连接配置指南)
9. [错误处理与容错指南](#错误处理与容错指南)
10. [yzf.service.call 通用调用分发表](#yzfservicecall-通用调用分发表)
11. [权限、安全与审计日志](#权限安全与审计日志)
12. [Rhino引擎注意事项与陷阱](#12-rhino-引擎注意事项与陷阱)
13. [服务端命令参考](#13-服务端命令参考)
14. [完整模块模板](#14-完整模块模板)
15. [JVM启动参数](#15-jvm-启动参数)
16. [FAQ](#16-faq)
17. [内置模块、插件与脚手架](#17-内置模块插件与脚手架)
...
每个模块有独立的配置存储，位于模块 `data/config.json`。
...
```
yzf modules                 # 列出所有已加载模块
yzf plugins                 # 列出所有插件
yzf reload                  # 重载全部模块
yzf reload <moduleId>       # 重载指定模块（如 "author/module"）
yzf enable <moduleId>       # 启用模块
yzf disable <moduleId>      # 禁用模块
yzf plugin enable <id>      # 启用插件
yzf plugin disable <id>     # 禁用插件
yzf info <moduleId>         # 查看模块详细信息
```
...
```
yzf status                  # 显示系统状态概览
yzf metrics                 # 查看性能指标（加载次数、失败次数、命令调用次数等）
yzf audit                   # 查看审计日志
```
...
```
yzf permission <player> <permission>   # 设置玩家权限
```
...
**Q: 数据存储在哪里？**
A: `yzf.config.*` 存储在模块的 `data/config.json`，`yzf.data.*` 存储在全局 `data/player-data/{comid}.json`。
...
| **UHD Render Framework** | Codex | 通用 HUD 渲染框架 | `月子芳/yueyu-hud` |
