...
public class NetServer implements ApplicationListener{
    ...
    public static void clientSnapshot(
        ...
        if(con == null) return;
        if(snapshotID < con.lastReceivedClientSnapshot){
            con.syncDroppedSnapshots++;
            YZFNetworkMetrics.recordDroppedSnapshot();
            return;
        }

        con.syncClientSnapshots++;
        YZFNetworkMetrics.recordClientSnapshot();
        ...
        if(!player.dead()){
            ...
            if(!ignorePosition){
                ...
                if(!verifyPosition){
                    ...
                    float correctionError = Mathf.dst(x, y, newx, newy);
                    con.syncCorrectionCount++;
                    con.syncLastPositionError = correctionError;
                    YZFNetworkMetrics.recordSyncCorrection(correctionError);
                    if(correctionError >= YZFNetSync.rubberbandDistance()){
                        con.syncRubberbandCount++;
